...
**Companion TCP access:** Supply Depot publishes host TCP port `8511` to port `5001` inside openHop for external Companion clients, alongside the web interface on host port `8510`. To use it, configure the intended Companion frame server with `tcp_port: 5001` and `bind_address: 0.0.0.0`, then connect to your NOMAD host on port `8511`. Publishing the port does not create a Companion identity or enable its server. Keep this endpoint on a trusted LAN; do not forward it from the internet. A plugin running inside the same container can still connect through `127.0.0.1:5001`.
